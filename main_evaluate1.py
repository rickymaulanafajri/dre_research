...
def get_args():
    ...
    parser.add_argument('--metric', default="mcp", type=str, choices=["random", "bald", "dfal", "entropy", "gini", "entropyDrop", "lc", "margin", "mcp", "egl", "kcenter", "dre","kcenterOpt"], help="name of acquisition functions")
...
def main():
    ...
    print('Stage \t Epoch \t TestAcc\t TestRobust')
    model.eval()
    if type == "full":
        ...
        #print('%d \t %d \t %.4f \t %.4f', 1, best_epoch, test_acc, test_attack_acc)
        print('%d \t %d \t %.4f \t %.4f' % (1, best_epoch, test_acc, test_attack_acc))

    elif type == "al":
        print("masuk sini")
        #attack_list = ["pgd", "vni","square","jitter","fab","pixle","cw"]
        attack_list = ["vni", "fab","jitter","pgd","vni","fab","auto","pgd","fab","auto","jitter"]
        #attack_list = ["jitter"]
        stage_num = int((parameters.budget - parameters.num_initial) / parameters.num_label)
        print(stage_num)
        #print(f"Stage num {stage_num},parameter budget {parameters.budget} parameter initial {parameters.num_initial} num labels {parameters.num_label}")
        #print(parameters.budget)

        for stage in range(10):
            print("sudah disini")
            random_attack = attack_list[stage]
            model_path = parameters.save_model_root + f"al-{args.train}-{args.metric}-{stage}-{args.ite}.pt"
            model, _, best_epoch, _, _, _ = load_best(model_path, model, opt)
            test_acc = evaluate_standard(test_loader, model)
            test_attack_acc = evaluate_adv(test_loader, model, random_attack, dataName)
            print(random_attack)
            logger.info('%d \t %d \t %.4f \t %.4f', stage, best_epoch, test_acc, test_attack_acc)
            #print('%d \t %d \t %.4f \t %.4f', stage, best_epoch, test_acc, test_attack_acc)
            print('%d \t %d \t %.4f \t %.4f' % (1, best_epoch, test_acc, test_attack_acc))
    else:
